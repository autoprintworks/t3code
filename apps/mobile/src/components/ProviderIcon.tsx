...
/**
 * Every provider glyph this client can draw, one per key in the contract's
 * `PROVIDER_ICON_KEYS`.
 *
 * *Which* key an instance draws is not decided here. `resolveProviderIconKey`
 * in `@t3tools/contracts` owns that rule so the web client and this one cannot
 * answer it differently for the same instance; this file only turns the answer
 * into artwork. A key with no entry here draws nothing, which is the honest
...
export function ProviderIcon(props: ProviderIconProps) {
  ...
  switch (iconKey) {
    ...
    // A command prompt: "some agent, and its own operator knows which".
    case "terminal": {
      return (
        <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
          <Path
    ...
    // The protocol's own mark, and the glyph a configured agent wears until its
    // user picks another. It names no vendor, which is the point of it.
    case "acp": {
      return (
        <Svg width={size} height={size} viewBox="0 0 576 220" fill="none">
          <Path
            fill={mono}
    ...
    // The four-point spark, flat. The web client fills the same shape through a
    // stack of blurred gradients; at a row's icon size that reads as one colour,
    // and a flat fill is the shape without the machinery.
    case "gemini": {
      return (
    ...
    case "trae": {
      return (
        <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
          {/* Back rectangle: a left strip and a bottom strip, the gap between
              them being the open bottom-left corner. */}
          <Rect x="1" y="4" width="3" height="14" fill={mono} />
          <Rect x="4" y="18" width="18" height="3" fill={mono} />
          {/* Front frame: top bar and right bar only. */}
          <Rect x="4" y="4" width="18" height="3" fill={mono} />
          <Rect x="19" y="7" width="3" height="11" fill={mono} />
          <Path fill={mono} d="M11 10L13 12L11 14L9 12Z" />
          <Path fill={mono} d="M16 10L18 12L16 14L14 12Z" />
        </Svg>
      );
    }
    ...
    case "antigravity": {
      // No path form exists for this one; the web client embeds the same bitmap
      // the same way.
      return (
        <Svg width={size} height={size} viewBox="0 0 128 128" fill="none">
          <Image href={{ uri: ANTIGRAVITY_ICON_DATA_URL }} width="128" height="128" />
        </Svg>
      );
    }
    ...
    default: {
      // A driver with no glyph of its own, or a key from a newer server. Both
      // are better served by nothing than by another vendor's mark.
      return null;
    }
  }
}
