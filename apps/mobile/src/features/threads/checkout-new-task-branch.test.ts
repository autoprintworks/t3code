...
beforeEach(async () => {
  ...
  // A host with `core.autocrlf=true` (the Git for Windows default) would
  // restore "feature\n" as "feature\r\n" on checkout, so the assertions below
  // would read the machine running the suite instead of the checkout under
  // test.
