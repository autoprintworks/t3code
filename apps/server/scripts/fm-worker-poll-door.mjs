/**
 * FORK DELTA (fm provider) - a stand-in First Mate ACP door, for
 * `bench-fm-worker-poll.ts`.
 *
 ...
 *
 * - `DOOR_WORKERS` - how many worker sessions `session/list` answers with.
 * - `DOOR_SILENT` - comma-separated methods to read and never answer.
 * - `DOOR_DROP_AFTER` - after this many `session/list` answers, stop listing
 *   the workers. Models a session that vanishes while its load is in flight.
 * - `DOOR_LIST_DELAY_MS` - milliseconds to wait before answering
 *   `session/list`, for measuring a slow door rather than an absent one.
 */
const WORKERS = Number(process.env.DOOR_WORKERS ?? "0");
const SILENT = new Set((process.env.DOOR_SILENT ?? "").split(",").filter(Boolean));
const DROP_AFTER = Number(process.env.DOOR_DROP_AFTER ?? "0");
const LIST_DELAY_MS = Number(process.env.DOOR_LIST_DELAY_MS ?? "0");

const SUPERVISOR = "fm-bench-supervisor";
let listCount = 0;

const workerRows = Array.from({ length: WORKERS }, (_unused, index) => ({
  sessionId: `fm-bench-worker-${String(index)}`,
...
const handle = (message) => {
  ...
  switch (method) {
    ...
    case "session/list": {
      ...
      const sessions = [
        { sessionId: SUPERVISOR, cwd: "/repo", title: "First Mate" },
        ...(dropped ? [] : workerRows),
      ];
      if (LIST_DELAY_MS > 0) {
