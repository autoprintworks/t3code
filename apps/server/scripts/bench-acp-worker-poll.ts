/**
 * The measurement harness for the worker-session poll.
 *
 * `docs/internals/acp-worker-threads.md` states four things about the poll that
 * a unit test cannot: what one poll costs, whether the Node event loop is free
 * while it runs, what happens when the agent accepts a request and never
 ...
 *
 * Usage:
 *   node apps/server/scripts/bench-acp-worker-poll.ts
 ...
 * `acp-worker-poll-agent.mjs` stands in for a real agent because the
 * measurement is of this side of the wire.
 */
...
const AGENT_SCRIPT = NodePath.join(
  NodePath.dirname(NodeURL.fileURLToPath(import.meta.url)),
  "acp-worker-poll-agent.mjs",
);
...
/** One run against a freshly spawned agent. */
const withRuntime = <A>(
  input: {
  ...
  Effect.gen(function* () {
    const context = yield* Layer.build(
      AcpSessionRuntime.layer({
        ...
        cwd: process.cwd(),
        clientInfo: { name: "bench-acp-worker-poll", version: "0.0.0" },
        authMethodId: "none",
...
const measurePoll = (workers: number) =>
  Effect.gen(function* () {
    ...
    const result = yield* withRuntime(
      {
        env: { AGENT_WORKERS: String(workers) },
...
/**
 * Measurement 3: the agent accepts `session/load` and never answers it.
 *
...
const measureSilentLoad = Effect.gen(function* () {
  ...
  const outcome = yield* withRuntime(
    {
      env: { AGENT_WORKERS: "1", AGENT_SILENT: "session/load" },
      pollIntervalMs: INTERVAL_MS,
      onRequest: (event) => {
 ...
 *
 * "Gone" is decided by the agent answering `session/list` without it, so the
...
const measureVanishMidLoad = Effect.gen(function* () {
  ...
  return yield* withRuntime(
    {
      env: { AGENT_WORKERS: "1", AGENT_SILENT: "session/load", AGENT_DROP_AFTER: "2" },
      pollIntervalMs: INTERVAL_MS,
      onRequest: (event) => {
    ...
    (runtime) =>
      Effect.gen(function* () {
        ...
        // The next roster is the one the agent answered without the worker.
...
const program = Effect.gen(function* () {
  ...
  console.log("\n3. agent accepts session/load and never answers");
