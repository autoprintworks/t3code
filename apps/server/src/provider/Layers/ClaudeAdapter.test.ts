...
describe("ClaudeAdapterLive", () => {
  ...
  describe("skill mentions", () => {
    // Claude Code starts a skill only from a slash command in the last text
    // block, so the composer's `$name` chip has to reach the process as
    // `/name` in that block.
    ...
    const sendSkillTurn = (
      ...
      sendSkillTurnMessage(text, modelSelection ? { modelSelection } : {}).pipe(
        // Reads the last text block, the one Claude Code inspects for a
    ...
    it.effect("sends a chip-only message as a slash command block", () =>
      Effect.gen(function* () {
        const message = yield* sendSkillTurnMessage("$to-spec");
        assert.deepEqual(message?.message.content, [{ type: "text", text: "/to-spec" }]);
      }),
    );
    ...
    // A project skill picked next to an attachment still runs. The image block
    // goes first so the slash command stays in the last text block.
    it.effect("keeps the skill command last when the turn carries an attachment", () =>
      Effect.gen(function* () {
        ...
        assert.equal(blocks[1]?.text, "/to-spec route A");
      }),
    );
    ...
    // A turn that is not a slash command keeps the single text block every
    // Claude turn had before skills existed.
    ...
    // The ultrathink prefix keeps its own leading block, so the slash command
    // is still the last block and Claude Code still expands it.
    it.effect("keeps the slash command last under an ultrathink prefix", () =>
      Effect.gen(function* () {
        const message = yield* sendSkillTurnMessage("$to-spec route A", {
          modelSelection: createModelSelection(
            ProviderInstanceId.make("claudeAgent"),
            SYNTHETIC_CLAUDE_CAPABLE_MODEL,
            [{ id: "effort", value: "ultrathink" }],
        ...
        assert.deepEqual(message?.message.content, [
          ...
          { type: "text", text: "/to-spec route A" },
        ]);
      }),
    );
  });
