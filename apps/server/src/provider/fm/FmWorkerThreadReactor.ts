 ...
 * steers a crewmate through First Mate, which is the one place that can
 * actually do it.
 *
 ...
 */
interface FmWorkerThreadRecord {
...
const makeFmWorkerThreadReactor = Effect.gen(function* () {
  ...
  const workerQuery = yield* FmWorkerThreadQuery;
  ...
  const workerThreads = new Map<ThreadId, FmWorkerThreadRecord>();
    ...
    workerQuery
      ...
      .pipe(
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("fm.worker.thread.replay-ids-unavailable", {
            cause,
            workerThreadId,
  ...
  const liveWorkerThread = (input: {
    ...
    readonly workerSessionId: string;
  }): { readonly threadId: ThreadId; readonly record: FmWorkerThreadRecord } | undefined => {
    const threadId = fmWorkerThreadId(input);
  ...
  const flushBufferedText = (workerThreadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      Effect.catchCause((cause) =>
        Effect.logWarning("fm.worker.text.flush-failed", { cause, workerThreadId }),
      ),
    );
   ...
   * on the worker ending, and on the watch ending - "the door said so" and
  ...
  const completeOpenMessage = (workerThreadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      yield* engine
        ...
        .pipe(
          Effect.catchCause((cause) =>
            Effect.logDebug("fm.worker.text.complete-skipped", { cause, workerThreadId }),
          ),
        );
    });
  ...
  const sweepStaleWorkerThreads = (input: {
    ...
    Effect.gen(function* () {
      const existing = yield* workerQuery.listActiveThreadIdsByPrefix({
        prefix: fmWorkerThreadIdPrefix(input.homeSessionId),
      ...
      for (const workerThreadId of existing) {
        ...
        // Not listed by a door that is answering, and not one this process is
        // watching: the worker ended while the editor was not running.
        yield* archiveWorkerThread(workerThreadId);
      }
    }).pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning("fm.worker.sweep-failed", { cause, homeSessionId: input.homeSessionId }),
      ),
    );
      ...
      switch (observation._tag) {
        case "WorkerAppeared": {
          yield* ensureWorkerThread({
            workerThreadId: fmWorkerThreadId(observation),
        ...
        case "WorkerDisappeared": {
          ...
          if (observation.reason === "unknown") {
            // The watch ended, not the worker. The fm daemon outlives the
          ...
          yield* engine
            .dispatch({
              ...
              activity: {
                ...
                tone: "error",
                kind: "fm.worker.transcript-unavailable",
            ...
            .pipe(
              Effect.catchCause((cause) =>
                Effect.logDebug("fm.worker.load-failed-note-skipped", {
                  cause,
  ...
  interface FmWatchedAdapter {
    readonly adapter: ProviderAdapterShape<ProviderAdapterError>;
    readonly scope: Scope.Scope;
  }

  const watchedRef = yield* Ref.make(new Map<ProviderInstanceId, FmWatchedAdapter>());

  const reconcile = Effect.gen(function* () {
    const previous = yield* Ref.get(watchedRef);
    const instanceIds = yield* registry.listInstances();
    const next = new Map<ProviderInstanceId, FmWatchedAdapter>();
