...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("t3-projection-transcript-order-")))(
  "OrchestrationProjectionPipeline transcript ordering",
  (it) => {
    it.effect("reads the transcript in sequence order when the host clock runs backwards", () =>
      Effect.gen(function* () {
        const projectionPipeline = yield* OrchestrationProjectionPipeline;
        const eventStore = yield* OrchestrationEventStore;
        ...
        const sql = yield* SqlClient.SqlClient;
        const threadId = ThreadId.make("thread-skew");

        // Every event carries a wall clock EARLIER than the one before it, the
        // shape a host clock excursion leaves behind. Only the sequence, which
        // the event store assigns on append, still tells write order.
        yield* eventStore.append({
          type: "thread.message-sent",
          eventId: EventId.make("evt-skew-1"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-02-26T12:00:09.000Z",
          commandId: CommandId.make("cmd-skew-1"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-skew-1"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.make("skew-user-1"),
            role: "user",
            text: "first",
            turnId: null,
            streaming: false,
            createdAt: "2026-02-26T12:00:09.000Z",
            updatedAt: "2026-02-26T12:00:09.000Z",
          },
        });

        yield* eventStore.append({
          type: "thread.turn-start-requested",
          eventId: EventId.make("evt-skew-2"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-02-26T12:00:08.000Z",
          commandId: CommandId.make("cmd-skew-2"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-skew-2"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.make("skew-user-1"),
        ...
        yield* eventStore.append({
          ...
          eventId: EventId.make("evt-skew-3"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-02-26T12:00:07.000Z",
          commandId: CommandId.make("cmd-skew-3"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-skew-3"),
          metadata: {},
          payload: {
            threadId,
            proposedPlan: {
              id: "skew-plan-1",
              turnId: TurnId.make("turn-skew"),
              planMarkdown: "# Ship it",
              implementedAt: null,
              implementationThreadId: null,
              createdAt: "2026-02-26T12:00:07.000Z",
              updatedAt: "2026-02-26T12:00:07.000Z",
            },
          },
        });
        ...
        yield* eventStore.append({
          type: "thread.message-sent",
          eventId: EventId.make("evt-skew-4"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-02-26T12:00:05.000Z",
          commandId: CommandId.make("cmd-skew-4"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-skew-4"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.make("skew-assistant-1"),
            role: "assistant",
            text: "on it",
            turnId: TurnId.make("turn-skew"),
            streaming: false,
            createdAt: "2026-02-26T12:00:05.000Z",
        ...
        yield* eventStore.append({
          type: "thread.message-sent",
          eventId: EventId.make("evt-skew-5"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-02-26T12:00:03.000Z",
          commandId: CommandId.make("cmd-skew-5"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-skew-5"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.make("skew-user-2"),
            role: "user",
            text: "second",
            turnId: null,
            streaming: false,
            createdAt: "2026-02-26T12:00:03.000Z",
            updatedAt: "2026-02-26T12:00:03.000Z",
          },
        });

        yield* projectionPipeline.bootstrap;

        const messages = yield* messageRepository.listByThreadId({ threadId });
        assert.deepEqual(
          messages.map((message) => message.messageId),
          ["skew-user-1", "skew-assistant-1", "skew-user-2"],
        );
        const messageSequences = messages.map((message) => message.sequence ?? 0);
        assert.deepEqual(
          messageSequences,
          [...messageSequences].sort((left, right) => left - right),
        );
        assert.equal(new Set(messageSequences).size, messageSequences.length);

        const plans = yield* planRepository.listByThreadId({ threadId });
        assert.equal(plans.length, 1);
        // The plan was written between the two messages, so its sequence sits
        // between theirs even though its timestamp is the second-newest.
        assert.equal((plans[0]?.sequence ?? 0) > (messageSequences[0] ?? 0), true);
        assert.equal((plans[0]?.sequence ?? 0) < (messageSequences[1] ?? 0), true);

        // The pending turn anchors on its opening user message, not on the
        // later turn-start-requested event, so the turn's own first message
        // stays on the turn's page rather than falling onto the previous one.
        const pendingTurnRows = yield* sql<{
          ...
          FROM projection_turns
          WHERE thread_id = ${threadId}
            AND pending_message_id = 'skew-user-1'
        `;
        assert.equal(pendingTurnRows.length, 1);
        assert.equal(pendingTurnRows[0]?.sequence, messageSequences[0]);

        // Every turn row carries a real anchor; a zero would sort a turn ahead
        // of the whole transcript and break the keyset walk.
        const turnRows = yield* sql<{
          readonly sequence: number;
        }>`
          SELECT sequence
        ...
        `;
        assert.equal(
          turnRows.every((row) => row.sequence > 0),
          true,
        );
      }),
    );
