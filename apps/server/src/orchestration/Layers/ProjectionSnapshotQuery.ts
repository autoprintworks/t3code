 ...
 * the reactor records the new one.
 */
...
function mapProjectShellRow(
  ...
  return {
    ...
    repositoryIdentity: storedRepositoryIdentity(row),
    ...
    execute: () =>
      sql`
        SELECT
          ...
          repository_identity_workspace_root AS "repositoryIdentityWorkspaceRoot",
...
pending_approval_requests AS (
          ...
          Effect.gen(function* () {
            ...
            const snapshot = {
              ...
              projects: Arr.filterMap(projectRows, (row) =>
                row.deletedAt === null ? Result.succeed(mapProjectShellRow(row)) : Result.failVoid,
              ),
            ...
            const snapshot = {
              ...
              projects: Arr.filterMap(projectRows, (row) =>
                row.deletedAt === null && activeProjectIds.has(row.projectId)
                  ? Result.succeed(mapProjectShellRow(row))
                  : Result.failVoid,
              ),
    ...
    getActiveProjectRowById({ projectId }).pipe(
      ...
      Effect.flatMap((option) =>
        Option.isNone(option)
          ...
          : Effect.succeed(Option.some(mapProjectShellRow(option.value))),
      ),
    );
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        readOnly: threadRow.value.readOnly !== 0,
  ...
  const getThreadDetailByIdBounded = (
    ...
    Effect.gen(function* () {
      ...
      const [messageRows, proposedPlanRows, activities, checkpointRows, latestTurnRow, sessionRow] =
        yield* Effect.all([
          (bounds === undefined
      ...
      const thread = {
        ...
        ...(threadRow.value.linkedPullRequest === null
          ...
          : { linkedPullRequest: threadRow.value.linkedPullRequest }),
        readOnly: threadRow.value.readOnly !== 0,
        fleetOwned: threadRow.value.fleetOwned !== 0,
        latestTurn: Option.isSome(latestTurnRow) ? mapLatestTurn(latestTurnRow.value) : null,
        createdAt: threadRow.value.createdAt,
        updatedAt: threadRow.value.updatedAt,
        archivedAt: threadRow.value.archivedAt,
