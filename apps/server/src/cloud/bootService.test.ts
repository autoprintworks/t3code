...
it.layer(NodeServices.layer)("boot service install", (it) => {
  // These simulate a Linux or macOS host so the platform guard lets writeDurably run, but
  // the filesystem underneath is the real one, and writeDurably ends with a directory fsync.
  ...
  onDisk("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
  ...
  onDisk("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
  ...
  onDisk("reconstructs a launch agent search path when the installer has no PATH", () =>
    Effect.gen(function* () {
  ...
  onDisk("adds missing provider directories to a minimal installer PATH", () =>
    Effect.gen(function* () {
  ...
  onDisk("keeps an installed launch agent current when the process PATH changes", () =>
    Effect.gen(function* () {
  ...
  onDisk("drops PATH directories that cannot be represented in a launch agent plist", () =>
    Effect.gen(function* () {
  ...
  onDisk("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
  ...
  onDisk("restarts without overwriting a pending remote update on macOS", () =>
    Effect.gen(function* () {
