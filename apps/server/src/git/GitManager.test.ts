...
/**
 * Read a file git itself checked out. A Windows host with `core.autocrlf=true`
 * writes CRLF into the worktree, so compare on the committed line endings.
 */
const readCheckedOutText = (filePath: string) =>
  NodeFS.readFileSync(filePath, "utf8").replaceAll("\r\n", "\n");
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("resets a clean reused PR worktree onto a force-pushed pull request head", () =>
    Effect.gen(function* () {
      ...
      expect(readCheckedOutText(NodePath.join(worktreePath, "force-pushed.txt"))).toBe(
        "rewritten\n",
      );
    }),
  );
