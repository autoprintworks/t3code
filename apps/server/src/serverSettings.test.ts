...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("rescues a legacy provider key this build no longer names", () =>
    Effect.gen(function* () {
      ...
      // The legacy block's own `enabled` is folded into the envelope and stripped
      // from the blob, so an instance carries exactly one enabled flag.
      assert.deepEqual(rescued?.config, { binaryPath: "/opt/fm/bin/fm" });
