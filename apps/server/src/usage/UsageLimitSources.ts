...
/**
 * How often the source list is re-read when nothing has changed.
 *
 * Upstream drives this loop from the provider health refresh interval setting.
 * The fork removed that setting because nothing else read it, so the period is
 * named here instead. It keeps upstream's five minutes.
 */
...
export const make = Effect.gen(function* () {
  ...
  const interval = Effect.succeed(USAGE_LIMIT_REFRESH_INTERVAL);
