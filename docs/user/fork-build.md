...
message. The only sign is the update pill label next to the update button in the sidebar, naming the
version that is waiting. Press that button if you want the new version now instead of on your next
quit.

Settings shows what the updater is doing in plain words, under **Update status**: checking for
updates, downloading with the percentage, a named version waiting for a restart, or up to date. The
row is not there until the first check has run, so it never says "Up to date." before it knows.

The setting is a desktop one. A browser or phone connected to this app over the network sees the
update state but cannot change the switch. Change it on the desktop machine that runs the updates.
