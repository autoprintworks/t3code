...
A skill marked `user-invocable: false` is meant for the agent only. It stays out
of the skill menu and out of the `/` command list, so those menus show what you
can actually pick. A shorter menu after an update is usually the skill author's
choice, not a fault. To get a skill back in the menu, remove `user-invocable:
false` from the top of its `SKILL.md` file.
