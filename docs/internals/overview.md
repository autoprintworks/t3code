...
Every `git` the environment runs for itself takes a permit from
[`GitWorkDepth`](../../apps/server/src/vcs/GitWorkDepth.ts): the VCS status subscriptions behind
[`VcsProcess`](../../apps/server/src/vcs/VcsProcess.ts) and the repository identity lookups in
[`RepositoryIdentityResolver`](../../apps/server/src/project/RepositoryIdentityResolver.ts). Each
spawn is a burst of filesystem work on the libuv threadpool, so without a bound a hundred watched
projects mean a hundred concurrent spawns and every unrelated request queues behind them. Depth, not
the number of open threads, is the lever.
...
not asked twice and two projects sharing a root cost one lookup. A project meta update that carries
a new workspace root refreshes the entry through
[`RepositoryIdentityReactor`](../../apps/server/src/orchestration/Layers/RepositoryIdentityReactor.ts),
which calls `resolve` with `refresh: true`.
