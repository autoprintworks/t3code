...
## The configurable ACP agent driver

The fork adds a sixth built-in driver, `acpAgent`, in
[`acpAgent/AcpAgentDriver.ts`](../../apps/server/src/provider/acpAgent/AcpAgentDriver.ts). It is not
one agent. Its command, arguments, working directory, environment, display name and icon all come
from the instance's settings, so several instances can be configured and each is a different agent
driven through the shared ACP runtime in `apps/server/src/provider/acp/`. Its behaviour contract is
the golden transcripts under `acpAgent/fixtures/acp-transcript/`, replayed at the real adapter by
`AcpAgentTranscript.test.ts`. The user-facing guide is
[external-acp-agents.md](../user/external-acp-agents.md).

Worker threads started through an ACP agent are read-only for the agent that owns them; see
[acp-worker-threads.md](./acp-worker-threads.md).
