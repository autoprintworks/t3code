...
A configured ACP agent can host more than one session on its connection: the
one T3 Code opened, and any others the agent started for itself. An agent that
delegates - a supervisor handing work to sub-agents - is the common case. This
document is how those extra sessions become threads, and - more important - why
doing it cannot make the editor slow.

Nothing here is specific to any one agent. The whole feature turns on
`sessionCapabilities.list`: an agent that advertises it gets its peer sessions
mirrored, an agent that does not is never asked.

## The shape

```
configured agent (one connection, one home)
  session/list  ->  our own session + N peer sessions
                        |
                        v
  AcpSessionRuntime     poll -> AcpPeerSessions.diff -> PubSub
                        |
                        v
  AcpAgentAdapter       AcpAgentWorkerObservation stream
                        |
                        v
  AcpAgentWorkerThreadReactor
                        thread.create (readOnly) / message deltas / archive
```
...
- **[AcpAgentWorkerSessions.ts](../../apps/server/src/provider/acpAgent/AcpAgentWorkerSessions.ts)**
  is the pure layer above it: thread ids, titles, message ids, and the
  reconcile that turns a roster into "appeared" and "disappeared".
- **[AcpAgentWorkerThreadReactor.ts](../../apps/server/src/provider/acpAgent/AcpAgentWorkerThreadReactor.ts)**
...
- **Gated.** The poll parks on a `Latch` that the first
  ...
  runtime nobody is watching sends no `session/list` at all, whatever the agent
  advertises. `AcpAgentWorkerAgent.test.ts` asserts a started runtime with no
...
- **Bounded.** `session/list` has a 10 s timeout, a peer `session/load` has
  ...
  from the end of the previous attempt, so a slow agent throttles the poll
  rather than stacking requests on it. The answer itself has a ceiling too:
  `MAX_PEER_SESSIONS` is 500, and an agent listing more is cut to it with one
  `acp.peer-sessions.ceiling` warning per poll. The length of the agent's
  answer is the agent's choice, and everything downstream of it is per-session
  work.
...
- **Off the request path.** Nothing in the loop spawns a process, calls `git`,
  ...
  both through [AcpAgentWorkerThreadQuery.ts](../../apps/server/src/provider/acpAgent/AcpAgentWorkerThreadQuery.ts),
  both with a `LIMIT`.

## Identity, and what survives a restart

**Thread id.** `acp-worker.<homeSessionId>.<workerSessionId>`, each segment
sanitised and truncated to 64 characters with a short hash appended when it is
cut. It is keyed on the **home** - the agent's own session - not on the
supervisor thread, so two threads open on one home adopt the same worker thread
instead of creating two.

**Message id.** The agent's own item id, passed through unchanged. A restart
...
holds, so a replay writes nothing twice. Only when the agent gives no item id
...
| --------------------------------------------- | ---------- | --------------------------- |
| The agent answered `session/list` without it  | `finished` | The thread is archived      |
| The watch itself ended (shutdown, agent exit) | `unknown`  | The thread is left alone    |
...
anything. A load that fails is terminal for that worker: the thread gets an
`acp.worker.transcript-unavailable` activity with an `error` tone, and nothing
tries again.
...
and proves nothing about wall-clock cost. `scripts/bench-acp-worker-poll.ts`
runs the shipping `AcpSessionRuntime` against
`scripts/acp-worker-poll-agent.mjs`, a real spawned subprocess over a real
pipe:

```
node apps/server/scripts/bench-acp-worker-poll.ts --workers 0,50,200,500
```
...
- **The agent accepts `session/load` and never answers.** The load failed after
...
- **The session disappears while its load is in flight.** Reported gone after
  ...
  agent answering `session/list` without it, not by the load ever coming back.
