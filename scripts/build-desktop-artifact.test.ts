...
// The bundle self-containment guard refuses to report success when any
// directory above its probe root owns a node_modules. That root lives in the OS
// temp directory, which on a developer machine can sit under a home directory
// that has one. Hide only those host-owned candidates so the check reaches the
// probe; the fixture's own tree stays visible, and the ancestor walk keeps its
// own test.
const hostIsolatedFileSystem = Effect.gen(function* () {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const hostTempRoot = NodeOS.tmpdir();
  const isHostAncestorNodeModules = (candidate: string) => {
    if (path.basename(candidate) !== "node_modules") return false;
    return !path.relative(path.dirname(candidate), hostTempRoot).startsWith("..");
  };
  return {
    ...fs,
    exists: (candidate: string) =>
      isHostAncestorNodeModules(candidate) ? Effect.succeed(false) : fs.exists(candidate),
  };
});
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: `${resolveDesktopProductName("1.2.3")} 1.2.3 Installer`,
      ...
      // entry advertises MimeType=x-scheme-handler/t3code-fork; for OAuth deep links.
  ...
  it.effect("validates every ASAR-unpacked native in the packaged Windows payload", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const probeFs = yield* hostIsolatedFileSystem;
        ...
        const result = yield* validateWindowsPackagedPayload({
          ...
          targetArch: "x64",
        }).pipe(Effect.provideService(FileSystem.FileSystem, probeFs));
      ...
      // The packaged primary is a stub file here, so only a non-Windows host can
      // run this. Pin the host so a Windows developer sees the same skip.
  ...
  it.effect("probes fff through the packaged Windows primary instead of helper executables", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        // The self-containment probe inherits process.env, so a host that
        // already exports ELECTRON_RUN_AS_NODE makes that flag ambiguous. Match
        // on the packaged primary, which only this probe spawns.
  ...
  it.effect("skips the primary native probe for cross-architecture Windows payloads", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        // The self-containment probe inherits process.env, so a host that
        // already exports ELECTRON_RUN_AS_NODE makes that flag ambiguous. The
        // packaged primary is only ever spawned by the probe under test.
        assert.isFalse(
          commands.some(
            (command) =>
              command.command === path.join(fixture.packagedAppDir, fixture.appExecutableName),
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* validateWindowsPackagedPayload({
          ...
          appExecutableName: fixture.appExecutableName,
          targetArch: "x64",
        }).pipe(Effect.provideService(FileSystem.FileSystem, probeFs), Effect.flip);
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.autoprintworks.t3code",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.autoprintworks.t3code</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.autoprintworks.t3code");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/t3code.provisionprofile");
      // The sign path is joined with the host separator, so build the tail the
      // same way.
      const path = yield* Path.Path;
      assert.ok(String(mac.sign).endsWith(path.join("scripts", "sign-macos.ts")));
  ...
  it.effect("ships only Linux runtime members in the WSL archive", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        // Listed by name from its own directory: GNU tar reads an absolute
        // `C:\...` argument as a remote `host:path` and fails on Windows.
        const process = yield* spawner.spawn(
          ChildProcess.make("tar", ["-tzf", path.basename(archivePath)], {
