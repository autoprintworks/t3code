...
describe("applyThreadDetailEvent", () => {
  ...
  describe("thread.message-sent", () => {
    ...
    it("places a late-arriving older message by creation time, not arrival order", () => {
      const threadWithMessage: OrchestrationThread = {
        ...baseThread,
        messages: [
          {
            id: MessageId.make("msg-late"),
            role: "assistant",
            text: "Second",
            turnId: null,
            streaming: false,
      ...
      const result = applyThreadDetailEvent(threadWithMessage, {
        ...
        payload: {
          threadId: ThreadId.make("thread-1"),
          messageId: MessageId.make("msg-early"),
          role: "user",
          text: "First",
          turnId: null,
          streaming: false,
      ...
      expect(result.kind).toBe("updated");
      if (result.kind === "updated") {
        expect(result.thread.messages.map((message) => message.text)).toEqual(["First", "Second"]);
      }
    });

    it("breaks a same-instant tie by message id", () => {
      const threadWithMessage: OrchestrationThread = {
        ...baseThread,
        messages: [
          {
  ...
  describe("thread.activity-appended", () => {
    ...
    it("repairs snapshot ordering before fast-path appends engage", () => {
      ...
      // A null-sequence row predates the field, so `activityOrder` sorts it
      // first, the same placement the server projection uses. A snapshot array
      // that arrives in another order is re-sorted, not frozen as a prefix,
      // before the in-order live append lands.
      const result = applyThreadDetailEvent(
        {
          ...baseThread,
          activities: [makeActivity("activity-a", 1), makeActivity("activity-null", null)],
        },
        {
          ...baseEventFields,
          sequence: 135,
